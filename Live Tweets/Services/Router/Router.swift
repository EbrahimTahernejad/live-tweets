...
protocol RouterProtocol: ServiceProtocol {
    func present<ViewType: RootView>(view ContentView: ViewType.Type, input: ViewType.ViewModel.Input, output: ViewType.ViewModel.Output, presentationStyle: UIModalPresentationStyle, transitionStyle: UIModalTransitionStyle, animated: Bool, resetContext: Bool, completion: (() -> Void)?)
    
    func navigate<ViewType: RootView>(view ContentView: ViewType.Type, input: ViewType.ViewModel.Input, output: ViewType.ViewModel.Output, animated: Bool, completion: (() -> Void)?)
...
class Router: RouterProtocol {
    ...
    init<ViewType: RootView>(with ContentView: ViewType.Type, input: ViewType.ViewModel.Input, output: ViewType.ViewModel.Output, viewProvider: ViewProviderProtocol) {
        self.viewProvider = viewProvider
        viewProvider.update(router: self)
        let hostController = HostingController(rootView: viewProvider.provide(with: ContentView.self, input: input, output: output), viewProvider: viewProvider)
    ...
    func present<ViewType: RootView>(view ContentView: ViewType.Type, input: ViewType.ViewModel.Input, output: ViewType.ViewModel.Output, presentationStyle: UIModalPresentationStyle = .automatic, transitionStyle: UIModalTransitionStyle = .coverVertical, animated: Bool = true, resetContext: Bool = false, completion: (() -> Void)? = nil) {
        let hostController = HostingController(rootView: viewProvider.provide(with: ViewType.self, input: input, output: output), viewProvider: viewProvider)
        let nav = NavigationController(rootViewController: hostController)
        nav.navDelegate = self
    ...
    func navigate<ViewType: RootView>(view ContentView: ViewType.Type, input: ViewType.ViewModel.Input, output: ViewType.ViewModel.Output, animated: Bool = true, completion: (() -> Void)? = nil) {
        let hostController = HostingController(rootView: viewProvider.provide(with: ViewType.self, input: input, output: output), viewProvider: viewProvider)
